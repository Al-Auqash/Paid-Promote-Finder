    ...
    <div class="collapse navbar-collapse" id="navigation">
      <ul class="navbar-nav mx-auto">
        @if (auth()->user())
            ...
            <li class="nav-item">
            <a class="nav-link me-2" href="{{ url('dashboard/profile') }}">
        ...
        @endif
        <li class="nav-item">
          <a class="nav-link me-2" href="{{ auth()->user() ? url('dashboard/static-sign-up') : url('dashboard/register') }}">
            <i class="fas fa-user-circle opacity-6 me-1 {{ (Request::is('static-sign-up') ? '' : 'text-dark') }}"></i>
            Sign Up
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link me-2" href="{{ auth()->user() ? url('dashboard/static-sign-in') : url('dashboard/login') }}">
      ...
      <ul class="navbar-nav d-lg-block d-none">
        <li class="nav-item">
          <a href="/" target="_blank" class="btn btn-sm btn-round mb-0 me-1 bg-gradient-{{ (Request::is('static-sign-up') ? 'light' : 'dark') }}">Home</a>
        </li>
      </ul>
    </div>
  </div>
